...
const DEFAULT_HM_PLAN = {
  ...
  weeks: [
    ...
    { week:0, calWeek:22, year:2026, dates:"25–31 mei", phase:"Raceweek", phaseColor:"green", sessions:[
      ...
      {typeId:"long",desc:"Zone 1–2, activering",plannedKm:10,zone:"Z1–2"},
...
// ═══════════ COUNTDOWN HELPERS ═══════════
function countdownLabel(weekNum) {
  if (weekNum === 0) return "Raceweek 🏁";
  if (weekNum === 1) return "Nog 1 week";
  return `Nog ${weekNum} weken`;
}

function countdownPill(weekNum) {
  if (weekNum === 0) return "0";
  return `-${weekNum}`;
}

function countdownShort(weekNum) {
  if (weekNum === 0) return "0";
  return `-${weekNum}`;
}
...
function getMonthOfCalWeek(calWeek, year) {
  ...
  const date = new Date(year, 0, dayOffset + 3);
