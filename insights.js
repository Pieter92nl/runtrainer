...
function insightsPlan(plan) {
  ...
  const wd = plan.weeks.map(w => {
    const pKm = weekPlannedKm(plan, w), aKm = weekLoggedKm(plan, w), feels = [], l2H = [], aH = [];
    w.sessions.forEach((s, si) => { const l = getLog(plan.id, w.week, si); if (l) { feels.push(l.feel); const tp = getType(s.typeId); if (tp.hint?.includes("intervallen") && l.hr > 0) l2H.push(l.hr); else if (l.hr > 0) aH.push(l.hr); } });
  ...
  plan.weeks.forEach(w => w.sessions.forEach((s,si) => {
    ...
    const l = getLog(plan.id, w.week, si); if (l) typeTotals[s.typeId].a += l.km || 0;
  }));
  ...
  <div class="icard"><h3>Gepland vs. werkelijk</h3>
    <div style="display:flex;align-items:flex-end;gap:4px;height:130px">${wd.map((w,i) => {
      const hP = Math.round(w.pKm/mx*100), hA = w.aKm ? Math.round(w.aKm/mx*100) : 0, pc = PHASE_COLORS[w.pc] || PHASE_COLORS.grey;
  ...
  <div class="icard"><h3>Per sessietype</h3>${Object.entries(typeTotals).map(([tid,v]) => {
    ...
    return `<div style="margin-bottom:14px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"><span style="font-size:14px;font-weight:600">${tp.icon} ${tp.name}</span><span style="font-size:13px;font-family:var(--mono);font-weight:700">${v.a.toFixed(1)} <span style="color:var(--t3);font-weight:500">/ ${v.p.toFixed(1)}</span>${v.a>0?` <span style="font-size:11px;color:${dc}">${d>=0?"+":""}${d.toFixed(1)}</span>`:""}</span></div><div style="height:5px;background:var(--s3);border-radius:3px;overflow:hidden"><div style="height:100%;width:${pc}%;background:${tp.color};border-radius:3px"></div></div></div>`;
  }).join("")}</div>

  <div class="icard"><h3>Hartslag trend</h3>
    <div style="font-size:12px;color:var(--t3);margin-bottom:12px">LT2 = interval-HR · Overig = hele activiteit</div>
    ${wd.some(w=>w.l2HR||w.aHR)?`<div class="legend" style="margin-top:0;margin-bottom:14px"><span><i style="background:var(--orange)"></i>LT2</span><span><i style="background:var(--blue)"></i>Overig</span></div>
    <div style="display:flex;align-items:flex-end;gap:4px;height:100px">${wd.map((w,i) => {
      ...
      return `<div style="flex:1;position:relative;height:100%">${dots.map(d=>{const p=Math.max(0,Math.min(100,(d[0]-120)/(195-120)*100));return`<div style="position:absolute;bottom:${p}%;left:50%;transform:translate(-50%,50%)"><div style="width:9px;height:9px;border-radius:50%;background:${d[1]}"></div><div style="font-size:8px;font-weight:700;color:var(--t2);font-family:var(--mono);text-align:center;margin-top:1px">${d[0]}</div></div>`;}).join("")}<div style="position:absolute;bottom:-16px;left:50%;transform:translateX(-50%);font-size:10px;font-weight:${i===planWeekIdx?800:600};color:${i===planWeekIdx?"var(--orange)":"var(--t3)"}">${countdownShort(w.w)}</div></div>`;
    }).join("")}</div><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--t3);margin-top:20px;font-family:var(--mono)"><span>120</span><span>195 bpm</span></div>`:`<div style="color:var(--t3);text-align:center;padding:20px">Log sessies met hartslag</div>`}
  </div>
...
function insightsType() {
  ...
  return Object.entries(typeData).map(([tid,entries]) => {
    const tp = getType(tid), sorted = entries.sort((a,b)=>(a.year*100+a.calWeek)-(b.year*100+b.calWeek));
    ...
    return `<div class="icard"><h3>${tp.icon} ${tp.name} — ${sorted.length} sessies</h3>
      ...
      ${hrs.length>1?`<div style="display:flex;align-items:flex-end;gap:4px;height:50px;margin-top:8px">${sorted.filter(e=>e.hr>0).map(e=>{const mn=Math.min(...hrs)-5,mx=Math.max(...hrs)+5,h=Math.round((e.hr-mn)/(mx-mn)*45);return`<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px"><div style="font-size:8px;color:var(--t3);font-family:var(--mono)">${e.hr}</div><div style="width:100%;height:${h}px;background:${tp.color};border-radius:2px 2px 0 0"></div></div>`;}).join("")}</div>`:""}</div>`;
  }).join("");
}
...
function parsePaceStr(s) { if(!s) return null; const p=s.split(":"); if(p.length===2){const m=parseInt(p[0]),sec=parseInt(p[1]);if(!isNaN(m)&&!isNaN(sec))return m*60+sec;} return null; }
function fmtPace(secs) { if(!secs) return "–"; const m=Math.floor(secs/60),s=Math.round(secs%60); return `${m}:${s.toString().padStart(2,"0")}`; }
