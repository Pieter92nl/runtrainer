...
function weekPlannedKm(plan, w) {
  return w.sessions.reduce((s, sess) => s + (sess.plannedKm || 0), 0);
}
function weekLoggedKm(plan, w) {
  return w.sessions.reduce((s, _, i) => { const l = getLog(plan.id, w.week, i); return s + (l ? l.km : 0); }, 0);
}
...
function getAutoWeekIdx(plan) {
  const cw = getCurrentCalWeek(), cy = new Date().getFullYear();
...
async function findFile() {
  try {
    const r = await fetch(`${DAPI}/files?spaces=appDataFolder&q=name='${FNAME}'&fields=files(id)`, { headers: { "Authorization": "Bearer " + token } });
    if (!r.ok) { if (r.status === 401) { token = null; syncState = "off"; render(); } return null; }
    const d = await r.json(); return d.files?.length ? d.files[0].id : null;
  } catch (e) { return null; }
}
...
function renderNav() {
  ...
  $("nav").innerHTML = tabs.map(t =>
    `<button class="${tab === t.id ? "on" : ""}" onclick="switchTab('${t.id}')">
