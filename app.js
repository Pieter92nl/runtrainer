...
let state = {
  sessionTypes: [],
  plans: [],
  logs: {},          // "planId:weekNum:sessionIdx" → {km,hr,pace,feel,terrain,notes}
  activePlanId: null,
  settings: { profile: { lt2HR:"178–182 bpm", lt2Pace:"~4:10/km", maxHR:"~199 bpm", target:"1:30 – 1:33" } }
};
...
// ═══════════ LOG KEY ═══════════
function logKey(planId, weekNum, sessionIdx) {
  return `${planId}:${weekNum}:${sessionIdx}`;
}

function getLog(planId, weekNum, sessionIdx) {
  return state.logs[logKey(planId, weekNum, sessionIdx)] || null;
}

function setLog(planId, weekNum, sessionIdx, entry) {
  state.logs[logKey(planId, weekNum, sessionIdx)] = entry;
}

function delLogEntry(planId, weekNum, sessionIdx) {
  delete state.logs[logKey(planId, weekNum, sessionIdx)];
}
...
function weekPlannedKm(plan, weekData) {
  return weekData.sessions.reduce((s, sess) => s + (sess.plannedKm || 0), 0);
}

function weekLoggedKm(plan, weekData) {
  return weekData.sessions.reduce((s, _, i) => {
    const l = getLog(plan.id, weekData.week, i);
    return s + (l ? l.km : 0);
  }, 0);
}
...
function getAutoWeekIdx(plan) {
  const cw = getCurrentCalWeek();
  const cy = new Date().getFullYear();
  for (let i = 0; i < plan.weeks.length; i++) {
    if (plan.weeks[i].calWeek === cw && plan.weeks[i].year === cy) return i;
  }
  return 0;
}

function syncDot() {
  return `<span class="sync-dot ${syncState}"></span>`;
}
...
function initGoogle() {
  ...
  tokenClient = google.accounts.oauth2.initTokenClient({
    ...
    callback: onToken,
    error_callback: () => { syncState = "err"; render(); }
  });
  const st = localStorage.getItem("hm-gtoken");
  if (st) {
    try {
      const t = JSON.parse(st);
      if (t.exp > Date.now()) { token = t.tk; syncState = "ok"; loadDrive(); }
    } catch (e) {}
  }
  render();
}
...
function doLogout() {
  token = null; fileId = null; syncState = "off";
  localStorage.removeItem("hm-gtoken");
...
async function findFile() {
  try {
    const r = await fetch(`${DAPI}/files?spaces=appDataFolder&q=name='${FNAME}'&fields=files(id)`, {
...
async function loadDrive() {
  ...
  try {
    ...
    if (fileId) {
      const r = await fetch(`${DAPI}/files/${fileId}?alt=media`, { headers: { "Authorization": "Bearer " + token } });
      if (r.ok) {
        const d = await r.json();
...
async function saveDrive() {
  ...
  try {
    ...
    if (!fileId) {
      ...
      else { syncState = r.status === 401 ? (token = null, "off") : "err"; }
    } else {
...
function saveLocal() {
  try { localStorage.setItem("hm-v2", JSON.stringify(state)); } catch (e) {}
}
...
function save() {
  ...
  svT = setTimeout(saveDrive, 1000);
}

// ═══════════ MIGRATION ═══════════
function migrateOldData() {
  // Try to read old format logs
  try {
    const old = localStorage.getItem("hm-log");
    if (old && !localStorage.getItem("hm-v2-migrated")) {
      const oldLog = JSON.parse(old);
      // Old keys: "w10-0" → new: "hm-2026:10:0" (but week numbers shifted)
      // Old w10 = new w10, old w1 = new w0 for race, etc.
      const oldToNew = { 10: 10, 9: 9, 8: 8, 7: 7, 6: 6, 5: 5, 4: 4, 3: 3, 2: 2, 1: 0 };
      for (const [key, val] of Object.entries(oldLog)) {
        const m = key.match(/^w(\d+)-(\d+)$/);
        if (m) {
          const oldWeek = parseInt(m[1]);
          const si = parseInt(m[2]);
          const newWeek = oldToNew[oldWeek] !== undefined ? oldToNew[oldWeek] : oldWeek;
          const nk = logKey("hm-2026", newWeek, si);
          if (!state.logs[nk]) {
            state.logs[nk] = { ...val, terrain: val.terrain || "road" };
          }
        }
      }
...
function ensureDefaults() {
  // Ensure session types
  if (!state.sessionTypes.length) {
    state.sessionTypes = JSON.parse(JSON.stringify(DEFAULT_TYPES));
  }
  // Ensure HM plan exists
  if (!state.plans.find(p => p.id === "hm-2026")) {
...
function initApp() {
  ...
  try {
    const cached = localStorage.getItem("hm-v2");
    if (cached) {
...
function render() {
  if (!token) {
    $("nav").style.display = "none";
    $("app").innerHTML = `<div class="login-screen"><h1>HM <span>Trainer</span></h1>
    <p>Trainingsplanner met Google Drive sync</p>
    ...
    <p class="login-sub">Je trainingslog wordt veilig opgeslagen in je eigen Google Drive.</p></div>`;
    return;
  }
  $("nav").style.display = "flex";
  renderNav();
...
function renderNav() {
  const tabs = [
    { id: "plan", l: "Plan", d: "M3 4h18a2 2 0 012 2v14a2 2 0 01-2 2H3a2 2 0 01-2-2V6a2 2 0 012-2zm13-2v4M8 2v4M3 10h18" },
  ...
  $("nav").innerHTML = tabs.map(t =>
    `<button class="${tab === t.id ? "on" : ""}" onclick="tab='${t.id}';render()">
