...
function renderPlan() {
  ...
  if (!plan) {
    $("app").innerHTML = `<div class="section" style="text-align:center;padding-top:80px">
      <p style="color:var(--t2);font-size:16px;margin-bottom:20px">Geen actief plan</p>
      <button onclick="tab='more';moreSection='builder';render()" class="btn-save" style="width:auto;padding:14px 28px">Plan aanmaken</button>
    </div>`;
    return;
  }
  ...
  if (!window._planInited || window._planInited !== plan.id) {
    planWeekIdx = getAutoWeekIdx(plan);
    window._planInited = plan.id;
  }
  ...
  <div class="hdr">
    <div class="hdr-row">
      <h1 style="font-size:20px">${plan.name} <span style="color:var(--orange)">W${w.week}</span></h1>
    ...
    <div class="pills" id="plan-pills">
      ${plan.weeks.map((wk, i) => {
        ...
        return `<button class="pill ${i === planWeekIdx ? "on" : ""} ${done ? "done" : ""}" onclick="planWeekIdx=${i};renderPlan()">W${wk.week}</button>`;
      }).join("")}
    </div>
  </div>
  <div class="phase">
    ...
    <span class="dt">${w.dates}${w.calWeek ? ` · KW${w.calWeek}` : ""}</span>
  </div>
  <div class="vol">
  ...
  <div class="cards">
    ${w.sessions.map((s, i) => planCardHTML(plan, w, s, i)).join("")}
  </div>
  <div style="height:24px"></div>`;

  // Scroll active pill into view
  setTimeout(() => {
    const pill = document.querySelector(".pill.on");
    if (pill) pill.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
  }, 50);
}

function planCardHTML(plan, week, sess, idx) {
  const type = getType(sess.typeId);
  const l = getLog(plan.id, week.week, idx);
...
function planLoggedHTML(l, type) {
  ...
  const terrainLabel = l.terrain === "trail" ? "🌲 Onverhard" : "";
  return `<div class="card-log">
    ...
    ${terrainLabel ? `<span class="tag">${terrainLabel}</span>` : ""}
...
// ═══════════ LOG SHEET ═══════════
const FE = ["😵", "😓", "😐", "😊", "🔥"];
let shPlanId, shWeek, shSi, shFeel = 3;
...
function openPlanLog(planId, weekNum, si) {
  ...
  const isInterval = type.hint?.includes("intervallen");
  const terrainVal = ex?.terrain || "road";

  $("sh").innerHTML = `
  <div class="sheet-bar"></div>
  <h2>${type.icon} ${type.name}</h2>
  <div class="sub">W${weekNum} — ${sess.desc}</div>
  <div class="hint ${isInterval ? "lt2" : "other"}" style="color:${type.color};background:${type.colorDim}">${type.hint || ""}</div>

  <div class="field">
    <label>${type.kmLabel || "Afstand"} (km)</label>
    <input type="number" step="0.1" id="f-km" value="${ex?.km ?? sess.plannedKm}" inputmode="decimal">
  </div>
  <div class="grid2">
    <div class="field">
      <label>${type.hrLabel || "Gem. HR"} (bpm)</label>
      <input type="number" id="f-hr" value="${ex?.hr ?? ""}" placeholder="—" inputmode="numeric">
    </div>
    <div class="field">
      <label>${type.paceLabel || "Gem. Pace"} (m:ss)</label>
      <input type="text" id="f-pace" value="${ex?.pace ?? ""}" placeholder="${type.pacePlaceholder || "5:30"}" inputmode="text">
    </div>
  </div>

  <div class="field">
    <label>Terrein</label>
    <div class="terrain-toggle" id="terrain-toggle">
      <button class="terrain-btn ${terrainVal === "road" ? "sel" : ""}" onclick="setTerrain('road')">🛣️ Verhard</button>
      <button class="terrain-btn ${terrainVal === "trail" ? "sel" : ""}" onclick="setTerrain('trail')">🌲 Onverhard</button>
    </div>
  </div>

  <div class="field">
    <label>Gevoel</label>
    <div class="feels">${[1, 2, 3, 4, 5].map(n =>
      `<button class="feel ${n === shFeel ? "sel" : ""}" onclick="pickFeel(${n})">${FE[n - 1]}</button>`
    ).join("")}</div>
  </div>

  <div class="field">
    <label>Notities</label>
    <textarea id="f-notes" placeholder="Hoe voelde het?">${ex?.notes ?? ""}</textarea>
  </div>
...
let currentTerrain = "road";
function setTerrain(val) {
  currentTerrain = val;
  document.querySelectorAll(".terrain-btn").forEach(b => {
    b.classList.toggle("sel", b.textContent.includes(val === "road" ? "Verhard" : "Onverhard"));
  });
}

function pickFeel(n) {
  shFeel = n;
  document.querySelectorAll(".feel").forEach((b, i) => b.classList.toggle("sel", i + 1 === n));
}

function savePlanLog() {
  setLog(shPlanId, shWeek, shSi, {
    km: parseFloat($("f-km").value) || 0,
    hr: parseInt($("f-hr").value) || 0,
    pace: $("f-pace").value.trim(),
    feel: shFeel,
...
function delPlanLog() {
  delLogEntry(shPlanId, shWeek, shSi);
  closeSheet();
  save();
  render();
}
