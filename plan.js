...
function renderPlan() {
  ...
  if (!plan) { $("app").innerHTML = `<div class="safe-top"></div><div class="section" style="text-align:center;padding-top:60px"><p style="color:var(--t2);font-size:16px;margin-bottom:20px">Geen actief plan</p><button onclick="tab='more';moreSection='builder';render()" class="btn-save" style="width:auto;padding:14px 28px">Plan aanmaken</button></div>`; return; }
  if (!window._planInited || window._planInited !== plan.id) { planWeekIdx = getAutoWeekIdx(plan); window._planInited = plan.id; }
...
function planCardHTML(plan, week, sess, idx) {
  ...
  return `<div class="card ${l?"logged":""}" onclick="openPlanLog('${plan.id}',${week.week},${idx})">
    ...
    <div class="card-metrics">
      ...
      ${isInt && sess.workKm ? `<span style="color:${type.color}">${workLabel}: ${sess.workKm} km</span>` : ""}
      ${isInt && sess.plannedMinutes ? `<span style="color:var(--t2)">${sess.plannedMinutes} min</span>` : ""}
    </div>
    ${sess.tip ? `<div class="card-tip" style="color:${type.color};background:${type.colorDim}">${sess.tip}</div>` : ""}
    ${l ? planLoggedHTML(l, type, sess) : ""}
  </div>`;
}
